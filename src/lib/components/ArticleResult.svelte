...
<a
    ...
    class="group background-opacity-10 focus-visible:preset-outlined-primary-500 focus-visible:ring-primary-500 preset-filled-surface-50-950 block overflow-hidden rounded-md shadow-sm transition-all duration-300 ease-in-out hover:shadow-md focus-visible:ring-2"
>
    <div class="p-5">
        {#if article.category}
            <p
        ...
        {#if article.headline}
            <h3
                class="h4 group-hover:text-primary-500 mb-2 text-ellipsis transition-colors duration-300"
            >
                {article.headline}
            </h3>
        {/if}
        <p class="text-surface-contrast-200-800 mb-3 text-sm opacity-95">
            {displayText}
        </p>
